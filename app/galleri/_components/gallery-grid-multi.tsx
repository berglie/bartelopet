    ...
    async (completionId: string, participantId: string) => {
      ...
      try {
        ...
        if (existingVote) {
          ...
          // Update vote count locally
          setCompletionsWithImages((prev) =>
            prev.map((c) => (c.id === completionId ? { ...c, vote_count: c.vote_count + 1 } : c))
          );
        }
              ...
              <CardHeader className="p-0">
                ...
                >
                  ...
                  {starredImage && completion.images.length > 1 && (
                    <div className="absolute left-2 top-2 rounded-full bg-accent p-1.5 text-accent-foreground shadow-lg">
                      <Star className="h-3 w-3 fill-current" />
                    </div>
                  )}

                  {/* Hover overlay - hidden on mobile */}
                  <div className="absolute inset-0 hidden items-center justify-center bg-black/0 transition-colors group-hover:bg-black/10 md:flex">
                ...
                {completion.images.length > 1 && (
                  <div className="hidden gap-1 bg-muted/50 p-2 md:flex">
                    {completion.images.slice(0, 4).map((img, imgIndex) => (
                      <button
                        key={img.id}
                        onClick={(e) => handleThumbnailClick(e, index)}
                        className="relative h-12 w-12 flex-shrink-0 cursor-pointer overflow-hidden rounded-sm border border-border transition-all hover:scale-105 hover:ring-2 hover:ring-accent"
                    ...
                    {completion.images.length > 4 && (
                      <button
                        onClick={(e) => handleThumbnailClick(e, index)}
                        className="flex h-12 w-12 cursor-pointer items-center justify-center rounded-sm border border-border bg-muted text-xs font-medium transition-all hover:scale-105 hover:ring-2 hover:ring-accent"
