 ...
 */
export async function deleteUserAccount(confirmationText: string): Promise<ActionResponse> {
  try {
    const supabase = await createClient();
    ...
    console.log(
      `[GDPR] User deletion request: ${user.id} - Email: ${user.email} - Participant: ${participant.id}`
    );
 ...
 */
export async function requestAccountDeletion(reason?: string): Promise<ActionResponse> {
  try {
    const supabase = await createClient();
